...
@tf.function
def train_step_MLP_intensity_loss(x, y, model, optimizer, intensity_loss_fn, train_acc_metric):
    """
    Example training step for a MLP model with a intensity loss function (trace mse).
    ...
    with tf.GradientTape() as tape:
        results = model(x, training=True)
        loss_value = intensity_loss_fn(y, results)
    grads = tape.gradient(loss_value, model.trainable_weights)
    optimizer.apply_gradients(zip(grads, model.trainable_weights))
...
@tf.function
def train_step_joint_loss_intensity(x, y, model, optimizer, weight_trace_loss, trace_loss_fn, weight_intensity_loss, intensity_loss_fn, trace_acc_metric, field_acc_metric, intensity_acc_metric):
    """
    Training step of a joint loss function (trace mse + intensity mse).
    ...
    Args:
        ...
        intensity_loss_fn (tf.keras.losses.Loss): Intensity loss function to use
        ...
        intensity_acc_metric (tf.keras.metrics.Metric): Metric to use for training accuracy
    """
    with tf.GradientTape() as tape:
        results = model(x, training=True)
        # Complex tensor of predictions
        ...
        loss_value = weight_trace_loss * trace_loss_fn(x, results) + weight_intensity_loss * intensity_loss_fn(y, results)
...
@tf.function
def train_step_combined_loss_training_intensity(x, y, model, optimizer, mode, trace_loss_fn, intensity_loss_fn, trace_acc_metric, field_acc_metric, intensity_acc_metric):
    """
    Training step of a combined training in which some steps are done with the trace loss function and
    others with the field loss function.
