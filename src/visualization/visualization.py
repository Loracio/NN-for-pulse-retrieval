...
from ..utils import meanVal, DFT, IDFT

class resultsGUI():
    def __init__(self, model, dataset, NUMBER_OF_PULSES, N, Δt):
        """
        ...
        self.true_electric_fields = tf.TensorArray(tf.complex64, size=NUMBER_OF_PULSES)

        index_to_write = 0
        for batch in dataset:
            for (trace, electric_field) in zip(batch[0], batch[1]):
                self.true_traces = self.true_traces.write(index_to_write, trace)
                complex_electric_field = tf.complex(electric_field[:N], electric_field[N:])
                self.true_electric_fields = self.true_electric_fields.write(index_to_write, complex_electric_field)
        ...
        self.predicted_electric_fields = tf.complex(self.predicted_electric_fields[:, :N], self.predicted_electric_fields[:, N:])

        # Initialize the fourier transform factors
        self.__init__fourier()

        # Get spectrums of the true and predicted electric fields
        self.true_spectrums = self.apply_DFT(self.true_electric_fields) #! Something weird happening here...
        self.predicted_spectrums = self.apply_DFT(self.predicted_electric_fields) #! Something weird happening here...

        # Compute the traces of the predicted electric fields
        self.predicted_traces = self.compute_trace(self.predicted_electric_fields)
    ...
    def __init__fourier(self,):
        ...
        Initialize the fourier transform factors
        """
        self.N_f = tf.cast(self.N, dtype=tf.float32) # Number of points in the trace (float)
    ...
    def plot(self,):
        ...
        self.next_button = Button(plt.axes([0.15, 0.6, 0.075, 0.04]), 'Next', hovercolor='aquamarine', color='lightcyan')
        self.next_button.on_clicked(self.next_result)
        self.prev_button = Button(plt.axes([0.05, 0.6, 0.075, 0.04]), 'Previous', hovercolor='aquamarine', color='lightcyan')
        self.prev_button.on_clicked(self.prev_result)


        plt.figtext(0.1, 0.9, 'Visualization mode', fontweight='bold')
        self.random_button = Button(plt.axes([0.025, 0.8, 0.075, 0.04]), 'Random', hovercolor='aquamarine', color='mediumseagreen')
        self.random_button.on_clicked(self.display_random)
        self.best_button = Button(plt.axes([0.125, 0.8, 0.075, 0.04]), 'Best', hovercolor='aquamarine', color='lightgrey')
        self.best_button.on_clicked(self.display_best)
        self.worst_button = Button(plt.axes([0.225, 0.8, 0.075, 0.04]), 'Worst', hovercolor='aquamarine', color='lightgrey')
        self.worst_button.on_clicked(self.display_worst)

        # self.best_index_order = np.argsort(self.errores_traza) # Best to worst
        # self.worst_index_order = np.flip(self.best_index_order) # Worst to best
        self.random_index_order = np.random.permutation(self.NUMBER_OF_PULSES) # Random order of the results

        self.last_index = 0
        self.previously_clicked = None
        self.mode = 'random'

        self.im0 = self.ax0.pcolormesh(self.omega, self.t, self.true_traces[0][:].reshape(self.N, self.N) / np.max(self.true_traces[0][:]), cmap='nipy_spectral')
        ...
        self.im1 = self.ax1.pcolormesh(self.omega, self.t, self.predicted_traces[0].reshape(self.N, self.N) / np.max(self.predicted_traces[0][:]), cmap='nipy_spectral')
        ...
        self.im2 = self.ax2.pcolormesh(self.omega, self.t, np.abs(self.true_traces[0] - self.predicted_traces[0]).reshape(self.N, self.N) , cmap='RdBu')
        ...
        predicted_spectrum = DFT(self.predicted_electric_fields[0], self.Δt)
        true_spectrum = DFT(self.true_electric_fields[0], self.Δt)

        I_predicted_field = np.abs(self.predicted_electric_fields[0])**2
        I_predicted_spectrum = np.abs(predicted_spectrum)**2

        I_true_field = np.abs(self.true_electric_fields[0])**2
        I_true_spectrum = np.abs(true_spectrum)**2

        predicted_field_phase = np.unwrap(np.angle(self.predicted_electric_fields[0])) 
        predicted_field_phase -=  meanVal(predicted_field_phase, I_predicted_field)
        # predicted_field_phase = np.where(I_true_field < 1e-10, np.nan, predicted_field_phase) #! Phase blanking

        predicted_spectrum_phase = np.unwrap(np.angle(predicted_spectrum)) 
        predicted_spectrum_phase -=  meanVal(predicted_spectrum_phase, I_predicted_spectrum)
        # predicted_spectrum_phase = np.where(I_true_spectrum < 1e-10, np.nan, predicted_spectrum_phase) #! Phase blanking

        true_field_phase = np.unwrap(np.angle(self.true_electric_fields[0])) 
        true_field_phase -=  meanVal(true_field_phase, I_true_field)
        # true_field_phase = np.where(I_true_field < 1e-10, np.nan, true_field_phase) #! Phase blanking

        true_spectrum_phase = np.unwrap(np.angle(true_spectrum)) 
        true_spectrum_phase -=  meanVal(true_spectrum_phase, I_true_spectrum) 
        # true_spectrum_phase = np.where(I_true_spectrum < 1e-10, np.true_spectrum_phase) #! Phase blanking


        self.line_I_true_field, = self.ax3.plot(self.t, I_true_field / np.max(I_true_field), color='blue', linewidth=3, alpha=0.5, label='True field intensity')
        self.line_true_field_phase, = self.twin_ax3.plot(self.t, true_field_phase, '-.', color='red',alpha=0.5)
        self.ax3.plot(np.nan, '-.', label='True phase', color='red')
        self.line_I_predicted_field, = self.ax3.plot(self.t, I_predicted_field / np.max(I_predicted_field), color='orange', label='Predicted field intensity')
        self.line_predicted_field_phase, = self.twin_ax3.plot(self.t, predicted_field_phase, '-.', color='violet')
        ...
        self.line_I_true_spectrum, = self.ax4.plot(self.omega, I_true_spectrum / np.max(I_true_spectrum), color='blue', linewidth=3, alpha=0.5, label='True spectral intensity')
        self.line_true_spectrum_phase, = self.twin_ax4.plot(true_spectrum_phase, '-.', color='red', alpha=0.5)
        self.ax4.plot(np.nan, '-.', label='True spectral phase', color='red')
        self.line_I_predicted_spectrum, = self.ax4.plot(self.omega, I_predicted_spectrum / np.max(I_predicted_spectrum), color='orange', label='Predicted spectral intensity')
        self.line_predicted_spectrum_phase, = self.twin_ax4.plot(self.omega, predicted_spectrum_phase, '-.', color='violet')
        self.ax4.plot(np.nan, '-.', label='Predicted spectral phase', color='violet')
    ...
    def update_plot(self):
        ...
        self.im0.set_array(self.true_traces[i].reshape(self.N, self.N) / np.max(self.true_traces[i]) )
        self.im0.set_clim(0, 1)

        max_Tpred = np.max(self.predicted_traces[i])
        if max_Tpred != 0:
            self.im1.set_array(self.predicted_traces[i].reshape(self.N, self.N) / max_Tpred)
            diff = np.abs(self.true_traces[i] / np.max(self.true_traces[i]) - self.predicted_traces[i] / max_Tpred).reshape(self.N, self.N)
        else:
            self.im1.set_array(np.zeros((self.N, self.N)))
            diff = np.abs(self.true_traces[i] / np.max(self.true_traces[i])).reshape(self.N, self.N)
        ...
        # Complex numbers to polar representation
        predicted_spectrum = DFT(self.predicted_electric_fields[i], self.Δt)
        true_spectrum = DFT(self.true_electric_fields[i], self.Δt)
        ...
        predicted_field_phase = np.unwrap(np.angle(self.predicted_electric_fields[i])) 
        predicted_field_phase -=  meanVal(predicted_field_phase, I_predicted_field)
        # predicted_field_phase = np.where(I_true_field < 1e-10, np.nan, predicted_field_phase) #! Phase blanking
        ...
        self.line_I_predicted_field.set_ydata(I_predicted_field / np.max(I_predicted_field))
        self.line_predicted_field_phase.set_ydata(predicted_field_phase)
        self.line_I_predicted_spectrum.set_ydata(I_predicted_spectrum / np.max(I_predicted_spectrum))
        self.line_predicted_spectrum_phase.set_ydata(predicted_spectrum_phase)

        self.line_I_true_field.set_ydata(I_true_field / np.max(I_true_field))
        self.line_true_field_phase.set_ydata(true_field_phase)
        self.line_I_true_spectrum.set_ydata(I_true_spectrum / np.max(I_true_spectrum))
