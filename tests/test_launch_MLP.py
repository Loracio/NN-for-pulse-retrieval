...
from src.models import MLP, train_MLP, trace_MSE, bottleneck_MLP
...
if __name__ == "__main__":
    ...
    config = {
        ...
        'n_hidden_layers': 6,
        'reduction_factor': 2,  # 'reduction_factor' is the factor by which the number of neurons is reduced in each layer in the bottleneck_MLP
        'n_neurons_per_layer': 4096,
        'activation': 'relu',
        'dropout': 0.1,
        'patience': 10,
        'training_size': 0.8,
    ...
    run = wandb.init(project="Reunión 09 Feb", config=config,
                    name='MLP_tanh_512_3layers_mseloss',)
    ...
    # Save the model
    model.save(f"./trained_models/FCNN/MLP.h5")